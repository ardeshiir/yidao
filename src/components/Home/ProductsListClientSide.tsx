...
const ProductsListClientSide = ({products:_products}:{products:Product[]}) => {
  return (
    <div
    ...
    >
      {products.map((prod, index) => (
        ...
        >
          <SingleProduct {...prod} price={_products?.[index]?.price || prod.price} />
        </motion.div>
      ))}
    </div>
  )
...
const SingleProduct = ({count,subtitle,title,price,description, image}:ProductsListProps) => {
  return (
    <div className={'px-[14px] relative md:max-h-[310px] py-[15px] bg-[#D9D9D94D] md:pt-[15px] pt-[200px] md:flex-row flex-col flex items-center justify-center gap-[50px] rounded-[30px]'}>
      <div className={'md:flex-grow md:h-full flex justify-center md:px-0 px-14 md:static absolute top-[-25%] md:w-auto min-h-full w-full'}>
        <div className={'relative aspect-[140/260] md:max-h-[270px] w-full h-full'}><Image quality={100}  src={image} alt="" fill={true} objectFit={'contain'} /></div>
      </div>
